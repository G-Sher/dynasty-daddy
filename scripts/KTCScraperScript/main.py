...
class Player:
    def __init__(self, id, name, first_name, last_name, team, position, positionRank, age, experience, sf_value, value, sleeperId=None):
        self.id = id
        self.name = name
    ...
    def toString(self):
        print(self.id, self.name, self.first_name, self.last_name, self.team, self.position, self.positionRank, self.age, self.experience, self.sf_value, self.value, self.sleeperId)
...
for player in sf_rankings:
    ...
    players.append(
        Player(playerId, playerName.text.strip(), playerFirstName, playerLastName, playerTeam.text.strip(), str(playerPosition.text.strip())[:2],
               None if str(playerPosition.text.strip())[2:] == 'CK' else str(playerPosition.text.strip())[2:], None if playerAge is None else str(playerAge.text.strip())[:2], playerExp,
...
try:
    ...
    try:
        ...
        for player in players:
            cursor.execute('''INSERT into ktc_players(name_id, sleeper_id, full_name, first_name, last_name, team, position, position_rank, age, experience, sf_trade_value, trade_value)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s )''', (player.id, player.sleeperId, player.name, player.first_name, player.last_name, player.team, player.position, player.positionRank, player.age, player.experience, player.sf_value, player.value))
